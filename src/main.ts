...
<section class="viewer" aria-label="谱面预览">
 ...
 <div class="stage" id="stage"><canvas id="chart-canvas" aria-label="三维谱面画布"></canvas><div class="stage-caption"><span>60 格原始坐标</span><span>程序化皮肤 / 非官方</span></div></div>
...
let frame=0;
function animate(){
 if(player&&renderer){const was=player.transport.playing,t=player.transport.time;if(was&&!player.transport.playing)player.pause();renderer.render(chart,t,speed,mirror,lines);input('timeline').value=String(t);el('time').textContent=`${format(t)} / ${format(chart.duration)}`;el('play').textContent=player.transport.playing?'Ⅱ 暂停':'▶ 播放';el('play').setAttribute('aria-label',player.transport.playing?'暂停':'播放');}
 frame=requestAnimationFrame(animate);
}animate();
window.addEventListener('pagehide',()=>{cancelAnimationFrame(frame);renderer?.dispose();player?.dispose();},{once:true});
