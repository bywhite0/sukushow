...
const applyRank=()=>{const v=el<HTMLSelectElement>('rank-preview').value;hud.setRankManual((v==='D'||v==='C'||v==='B'||v==='A'||v==='S'?v:'none') as 'none'|'D'|'C'|'B'|'A'|'S');};
el<HTMLSelectElement>('rank-preview').onchange=applyRank;applyRank();
const applyAppeal=()=>{const n=Number(el<HTMLInputElement>('opt-appeal').value);hud.setScoreConfig({totalAppeal:Number.isFinite(n)&&n>0?n:350000});};
el<HTMLInputElement>('opt-appeal').onchange=applyAppeal;applyAppeal();
