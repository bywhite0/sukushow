...
- **拖尾轨迹几何**：每段四边形现在连接实际历史位置，截面垂直于轨迹并朝向固定相机法线；不再用放在线段中点的竖直 billboard 代替斜向轨迹。零长度不绘制，沿视线方向退化时使用横向截面。尚未实现相邻段接缝与沿长度渐变，仍非完整 Unity TrailModule。拖尾独立材质槽已接入，shader 仍为预览近似，见下项。
