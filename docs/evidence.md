...
- **按路径细分发射**：`rateOverDistance` 在一帧内的 N 个粒子必须按位移等分铺开、各带自己的出生时刻与已存活时长。整帧一次性发射会让粒子全堆在帧末位置，且以整帧时长推进年龄而当场过期（首次实现即此错，靠逐粒子对账 `tmp/probe3.ts` 定位）。出生时刻还须钳在 `[prevAge, age] ∩ [0, CORE_MOVE_END]`：节点在 0.4166s 走完入场路径后不再位移，若仍把粒子铺到帧末，长帧跳转会在终点凭空堆出一簇。
...
- **`FeverEffectSet_001/LineParticle` 下 6 层已接入（贴图结论纠正）**：
  - ⚠ **旧稿错误**：曾把 `Particle_*`/`LineParticle_*` 的材质写成 `sharedassets5.assets:16`（`ComboInduceFrameEffect01`，并称「贴图缺失」）。根因是按 pathID 全局搜材质，撞上了 `sharedassets5.assets:16`（pathID 也是 16）。**判据**：`level56` 的 externals 表为 `[globalgamemanagers.assets, resources.assets, unity default resources, sharedassets56.assets, sharedassets0.assets]`，**不含 `sharedassets5.assets`** ⇒ level56 内的 PPtr 根本引用不到它。正确解析见下。
  - `LineParticle_L/R`（#603/#569）：**`ParticleSystemRenderer.m_Enabled = False`**（真 bool，非缺失字段）⇒ 原包不渲染。导出器已落成 `rend.off`，`compile()` 统一跳过（该字段原先无人读取，纯属侥幸不生效）。字段仍完整（`burst count=10`、`cycles=0`⇒无限、`interval=0.01`、寿命 0.2–0.5、`Shape` type=5 Box `pos=(∓6.87, 2.58, 0)` `scale=(0.5, 22, 1)` `rotz=∓45.41`、`Velocity.x=∓2.0`）。
  - 材质与贴图（逐 PPtr 解析，`m_FileID` 按 externals 索引）：`Closs_L/R`（#537/#568）→ `resources.assets:202` `sc2_Particle_CrosGlow`（`fid=2`）；`Particle_L/R`（#584/#571）与 `LineParticle_L/R` → `sharedassets56.assets:16` `sc2_Particle_light01`（`fid=4`），贴图 128×128、已随导出链路入库。
  - 6 层均为 `looping=True`、`lengthInSec=0.05`、burst `t=0 cycles=0`（无限）`interval=0.01`，`startDelay` 0–0.1，Transform 全为单位变换 ⇒ 与边线同帧、位于世界原点，是整个 Fever 期间的**稳态发射**（非一次性入场）。
  - 数据来源走**导出器单一真源**：`_rg_export_fx.py` 增加 `LineParticle_Left/Right`（root `297`/`233`）两个容器后重跑，材质表随之带出 `sharedassets56.assets:16`。自检：用导出器自身函数重生成 `feverLeft`，与已入库 `fx.json` 逐节点 `ps`+`rend` **全等 8/8**。
- **`ForceModule` 已接入**：`ForceOverLifetimeModule` 施加的是**加速度**（`v += F·dt`），不是速度叠加；`randomizePerFrame=False` ⇒ 每个粒子在**出生瞬间**采样一次并终生恒定；`inWorldSpace=False` 需按节点旋转折算到世界系（本子树各级旋转为单位 ⇒ 等价于世界轴）。6 层均为三轴 TwoConstants **±0.3**。此前预览完全未实现该模块，而 `fx.json` 早已导出 `force` 字段却在被静默忽略（另有 6 个已建模节点同样带 ±0.3 而未生效）。
- **`startDelay` 必须平移整个周期序列，不能折进 `bursts[].t`（已修正）**：`spawn()` 原先把采样到的 `startDelay` 加到每个 burst 的 `t` 上。当 `delay` 可能 ≥ `lengthInSec`（本子树 delay 0–0.1、period 0.05）时，`t` 恒大于周期 ⇒ 被周期窗口 `t >= period` 直接判掉 ⇒ **整层永不发射**。现改为 `Spec.delayAbs`，由 `emitDue` 平移周期边界：第 k 个周期区间是 `[delay + k·period, delay + (k+1)·period)`。已建模层不受影响（其 delay 远小于 period），但语义现在是正确的。
- **burst 批次的周期边界容差要按 `interval` 取相对量（已修正）**：`interval` 是 float32（0.01 → 0.009999999776），5 批累计 `0.04999999888`，比 `0.05` 略小 —— 固定 `1e-9` 容差放不住这个漂移，会让每周期多出边界重复的一批（100/s 变 120/s）。现用 `eps = max(1e-9, interval × 1e-3)`（远小于批间距，安全）。
